...
import { AggregationBatch, FeatureUnsucceedCount } from 'src/app/dashboard/state/dashboard.store';
...
export class ColumnComponent implements OnInit {
  ...
  ngOnInit() {
    this.colors = this.features.map((feature) => {
      return this.cellColor(this.column.feature_overall[feature]);
    });
  }
  ...
  cellColor(unsucceeded: FeatureUnsucceedCount) {
    if (unsucceeded == undefined) return 'lightgray';

    if (this.column == null) return;

    let { rows_count } = this.column;
    ...
    const failed_score = Math.min((failed / rows_count) * failed_multiplier, 1);
    const suspicious_score = Math.min((suspicious / rows_count) * suspicious_multiplier, 0.5);
